...
public class CommentController {
    ...
    public String deleteComment(@PathVariable Long commentId) {
        ...
        return "redirect:/books/" + comment.get().getBookId();  // ИСПРАВЛЕНО
    }
    ...
    public String updateComment(@PathVariable Long commentId, @Valid UpdateCommentRequestDto requestDto) {
        var comment = commentService.findById(commentId);
        var updateDto = new UpdateCommentDto(commentId, requestDto.text(), comment.get().getBookId());
        commentService.update(updateDto);
        return "redirect:/books/" + comment.get().getBookId();
    }
